...
- `src/catalog.ts`: experimental direct-HTTP catalog operations, typed table handles, metadata verification, and shared version checks for row mutations and B-tree index management. History preserves Python's newest-first order and cascade counts. Rollback returns a new handle using the caller's prior schema; it does not refresh the old handle. Column addition, renaming, removal, and computed-column creation return a new handle with the updated schema; the prior handle stays stale for writes. Mutation predicates use the protocol's Expr tag; query predicates remain embedded expression dictionaries.
- `src/catalog-query.ts`: immutable typed query builders, named expression projections with independent result schemas, and Python-compatible expression serialization, arithmetic type propagation, and update assignment validation. Scalar function declarations serialize import-path references and required named arguments; Python resolves and rebinds them. Function fixtures cover custom UDF serialization. Arithmetic fixtures cover every supported operator and column operands. Preserve nullability from both operands, and reject operands from different tables.
