...
Implemented: table/view/directory moves and removal, with dependency checks by default and explicit cascading removal. Typed version history and version-checked rollback of the latest data or schema change. Rollback returns a handle validated against the caller's expected prior schema.
