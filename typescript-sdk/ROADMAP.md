...
Implemented: an experimental adapter for direct protocol-v4 HTTP endpoints, with Python-compatible binary framing, directory creation, and recursive catalog listing. Non-persisting scalar/JSON computation returns values and cell errors, including view filtering. Scalar/JSON schemas, including dates and microsecond-preserving UTC timestamps, now support typed creation, schema-checked opening, insertion, literal and expression updates, primary-key batch updates/upserts, filtered deletes, collection with limits, and counts. Scalar column addition, renaming, and removal return updated typed handles. Insert error policies support abort (default) or retaining computation errors, with direct row counts and total error counts. Version checks reject stale writes without replay. Two-source inner, left, full outer, and cross joins accept same-client tables, inherited-column views, and snapshots with scoped expressions and nullable outer results. Self joins and chained joins remain unsupported. UUID columns use validated canonical branded strings, including primary keys, B-tree indexes, comparisons, membership, updates, and computed copies. Inline binary columns use Uint8Array with byte-preserving inserts, updates, batch updates, reads, computed copies, and compute results; binary expression literals use Python base64 encoding. Numeric/bool array schemas now support dtype and wildcard shape validation, typed-array inputs, CRUD, stored computed copies, and compute previews. Dimension-preserving array slices support projections and stored computed columns with inferred shape. Scalar arrayElement indexing requires declared dtype/rank and supports negative indices. Mixed integer/slice indexing also supports row and column selection with retained dimensions. Declared Python functions accept numeric/bool array literal arguments with exact JSON values. Array arithmetic, string arrays, remote references, and media remain incomplete. Distinct queries group the current projection, preserving result types and supporting group counts. Sample queries support fixed counts, fractions, stratification, and seeds with Python-compatible clause restrictions. Async cursor iteration uses one materialized proxy response; server streaming remains unavailable. Typed queries support grouped/global built-in sum, mean, min, max, and non-null count aggregates, partitioned/ordered running sum/min/max/count windows, JSON key/index/wildcard/slice paths and scalar/JSON casts with Python nullability, scalar membership in literal lists or JSON expressions, scalar comparisons, null checks, Boolean predicates, typed named expression projections, ordering, limits, and offsets. Numeric arithmetic with literal or same-table expression operands works in filters and updates. Stored scalar computed columns support backfills with abort or retained-error policies, typed handles, automatic maintenance, and read-only write types. Explicit B-tree indexes support creation and removal with version checks. Filtered live views inherit typed base columns, support computed columns, indexes, history, and nested views, and reflect base mutations. View handles omit row writes; schema mutations check versions across the base chain. Declared scalar Python function references work in projections and computed columns using required named arguments. Text embedding indexes reference installed Python UDFs and support similarity filtering, projection, and expression-based ranking. Signature discovery, custom aggregate functions, additional window/frame operations, projected/iterator views, multimodal embeddings, media, and schema diffs remain incomplete.
...
Implemented: read-only snapshots of tables and live views, including filtered creation and frozen base-chain reads; filtered recomputation with error-only retries, cascade control, and version checks; stored computed-column error type/message inspection through query expressions; table/view/directory moves and removal, with dependency checks by default and explicit cascading removal. Typed version history and version-checked rollback of the latest data or schema change. Rollback returns a handle validated against the caller's expected prior schema.
