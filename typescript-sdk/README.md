...
UUID columns use `{ type: 'uuid' }` and the branded `CatalogUuid` value returned by `catalogUuid(text)`. The helper accepts canonical hyphenated UUIDs, 32 hexadecimal digits, braced UUIDs, and the `urn:uuid:` prefix, normalizing to lowercase hyphenated form. All 128-bit values are supported, including nil UUIDs; malformed or whitespace-padded values are rejected. UUIDs support primary keys, B-tree indexes, comparisons, literal-list membership, ordering, computed copies, and compute previews. Python's built-in `min`/`max` do not accept UUID columns.

Array columns use `{ type: 'array', dtype: 'float32', shape: [null, 3] }`, where null dimensions are unconstrained. Omit shape to accept any shape, or omit both dtype and shape for any supported numeric array. Create values with `catalogArray(new Float32Array([1, 2, 3]))`; optionally pass a shape such as `[1, 3]`. Returned `CatalogArray` values expose `descr`, `shape`, `fortranOrder`, copied raw `data` bytes, `toNpy()`, and `toTypedArray()`. The typed-array read returns an independent native-endian, row-major flattened copy, converting Fortran order as needed. Int64/uint64 use BigInt typed arrays, float16 expands to Float32Array, and booleans use Uint8Array values 0/1. Shape remains on the CatalogArray. `CatalogArray.fromNpy(bytes)` imports numeric/bool NumPy files, including float16 and exact int64/uint64 values. Typed-array construction preserves its dtype; it does not cast values to fit the schema. Inserts, updates, batch updates, reads, computed copies, and compute previews validate dtype and shape. Use `table.columns.vector.arraySlice({ start: 1, stop: 5, step: 2 })` for server-side slicing. Pass a slice object or integer per axis; integers drop axes and omitted axes are retained. Integer indices require a declared shape, and arraySlice must retain at least one dimension; use arrayElement for scalars. Negative bounds/steps follow Python and slices can produce empty arrays. Slicing preserves nullability, infers known dimensions, and works in projections and stored computed columns. Use `arrayElement(-1)` for a vector scalar or `arrayElement(0, 1)` for a matrix scalar. It requires a declared dtype and shape, with one integer per dimension. Known bounds are checked locally; wildcard dimensions are checked by Python. Results are typed as number or boolean, plus null for nullable inputs; use `asType` to narrow an expression for arithmetic when needed. Scalar integer results must be safe JavaScript integers; retrieve the array and use `toTypedArray()` for full int64/uint64 precision. Numeric and boolean CatalogArray values can also be passed as literal arguments to declared Python functions; they serialize as nested values. Literal values must be finite, safe JSON numbers without negative zero. String arrays, array arithmetic, sorting, membership, and array indexes are not yet exposed. Python may reject non-native-endian inputs; the transport preserves their byte order.

Binary columns use `{ type: 'binary' }` and accept/return `Uint8Array` (including Node buffers). Empty arrays and byte-array slices retain their exact bytes. Inserts, updates, batch updates, collection, and `compute()` support binary values; stored computed columns can copy binary expressions. Equality predicates accept byte arrays. Row values travel in inline protocol parts, while expression literals use Python's base64 encoding. Binary sorting, membership lists, and indexes are not exposed. Remote object references and media payloads remain unsupported.

Query builders are immutable: filtering or selecting returns a new query. Predicates support comparisons, `isNull()`, `and()`, `or()`, and `not()`, using columns from the same table. Projections narrow the returned row type. Ordering supports scalar columns other than JSON and binary. `count()` counts matching rows and rejects queries with a limit or offset, matching Python. Two-source joins use their own expression scope, as shown below.
...
Summarize rows with `aggregate()` expressions and optional `groupBy()`:

```ts
const summaries = await sales
  .query()
  .groupBy('category')
  .selectExpressions({
...
Run the actual service test with a Python environment containing a compatible Pixeltable engine and its serving dependencies:
...
See [CAPABILITIES.md](CAPABILITIES.md) for Python API coverage and gaps, [ROADMAP.md](ROADMAP.md) for the subsequent phases and [CODEBASE_GUIDE.md](CODEBASE_GUIDE.md) for package maintenance.

The package includes a standalone GitHub workflow for Node 22 and 24 that runs formatting, types, unit tests, and a package dry run after extraction. The parent Pixeltable repository workflow additionally uses Python 3.12 and its frozen dependency lock, regenerates protocol/catalog fixtures, and runs live HTTP/catalog integration without external AI provider credentials. These checks do not establish compatibility with every published Pixeltable version or a hosted service.
