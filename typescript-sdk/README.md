...
Array columns use `{ type: 'array', dtype: 'float32', shape: [null, 3] }`, where null dimensions are unconstrained. Omit shape to accept any shape, or omit both dtype and shape for any supported numeric array. Create values with `catalogArray(new Float32Array([1, 2, 3]))`; optionally pass a shape such as `[1, 3]`. Returned `CatalogArray` values expose `descr`, `shape`, `fortranOrder`, copied raw `data` bytes, and `toNpy()`. `CatalogArray.fromNpy(bytes)` imports numeric/bool NumPy files, including float16 and exact int64/uint64 values. Typed-array construction preserves its dtype; it does not cast values to fit the schema. Inserts, updates, batch updates, reads, computed copies, and compute previews validate dtype and shape. String arrays, array expression literals, indexing, array arithmetic, sorting, membership, and array indexes are not yet exposed. Python may reject non-native-endian inputs; the transport preserves their byte order.
